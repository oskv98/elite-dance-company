        ...
        e.respondWith(
            fetch(e.request, { cache: 'no-store' })
                ...
                .catch(() => caches.match('./index.html').then(cached => cached || new Response(
                    ...
                    <style>body{background:#07030c;color:#fff;font-family:sans-serif;display:flex;
                        align-items:center;justify-content:center;min-height:100vh;margin:0;
                        text-align:center;padding:2rem;} h2{color:#ec2c6d;} p{color:#a99bb8;}</style>
...
// ── PUSH: los dos tipos de aviso llegan con distinto 'tag' ──
self.addEventListener('push', e => {
    let datos = {};
    try { datos = e.data ? e.data.json() : {}; } catch (err) { datos = {}; }

    const esUrgente = datos.tag === 'vencido';
    const titulo   = datos.title || (esUrgente ? '🚨 Mensualidad vencida' : '💜 Mensualidades por vencer');
    const opciones = {
        body: datos.body || '',
        icon: datos.icon || './icon-192.png',
        badge: './icon-192.png',
        tag: datos.tag || 'elite-aviso',
        vibrate: esUrgente ? [200, 100, 200, 100, 200] : [120],
        requireInteraction: esUrgente, // el aviso urgente se queda hasta que la toquen
        data: { url: esUrgente ? './?tab=vencimientos&filtro=vencidos' : './?tab=vencimientos&filtro=por-vencer' },
    };
